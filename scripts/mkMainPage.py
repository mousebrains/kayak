...
def mkRow(key, label):
    line = '<tr>'
    line+= '<th><a href="?P=' + key + '.html">' + label + '</a>&nbsp;</th>'
    line+= '<td><a href="?P=' + key + '.text">Text Version</a></td>'
    line+= '</tr>\n'
    return line

def mkIndex(tgt, fn, head, tail, logger): 
    logger.info('Making %s', fn) 
    states = mkStates(tgt)
    ...
    if tfn is not None and os.path.isfile(tfn):
        os.chmod(tfn, 0o664)
        os.rename(tfn, fn) 
    
parser = argparse.ArgumentParser()
parser.add_argument('-f', '--force', action='store_true', help='Force rebuilding main page')
MyDB.DB.addArgs(parser) # Database related options
MyLogger.addArgs(parser) # Logging related options
...
try:
    ...
    if qRebuild:
        mkIndex(cur, indexFile, loadFile(templateHead), loadFile(templateTail), logger)
