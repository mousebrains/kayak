...
private:
  ...
  double getDouble(const std::string& key) const {
    tRow::const_iterator it(mRow.find(key));
    if ((it == mRow.end()) || it->second.empty()) return NAN;
    const char *str(it->second.c_str());
    char *ptr;
    const double value(strtod(str, &ptr));
    if (!*ptr) return value;
    std::cerr << "Error converting '" << it->second << "' to a double for key(" << key << ")" << std::endl;
    return NAN;
  }

  double getFlow(const std::string& key) const {
    tRow::const_iterator it(mRow.find(key));
    if ((it == mRow.end()) || it->second.empty()) return NAN;
    const char *str(it->second.c_str());
    char *ptr;
    const double value(strtod(str, &ptr));
    return *ptr ? NAN : value;
  }
  ...
  double getGauge(const std::string& key) const {
    ...
    char *ptr;
    const double value(strtod(str, &ptr));
    const bool qConvert(*ptr && (ptr != str));
    return qConvert ? value : NAN;
  }

  void set(const std::string& key, const std::string& val) {mRow.insert(std::make_pair(key, val));}
...
public:
  ...
  double elevationLost() const {return getFlow("elevation_lost");}
  std::string features() const {return getString("features");}
  std::string location() const {return getString("gauge_location");}
  double gradient() const {return getFlow("gradient");}
  std::string guideBook() const {return getString("guide_book");}
  double highFlow() const {return getFlow("high_flow");}
  double highGauge() const {return getGauge("high_flow");}
  ...
  double lowFlow() const {return getFlow("low_flow");}
  double lowGauge() const {return getGauge("low_flow");}
  ...
  double optimalFlow() const {return getFlow("optimal_flow");}
  ...
  void gaugeName(const std::string& name) {set("gaugeName", name);}
  ...
  void pruneParsers(const tSet& tbls) {
    for (tSet::iterator it(mParsers.begin()), et(mParsers.end()); it != et;) { 
      if (tbls.find(*it) == tbls.end()) { // Not known
...
public:
  Master() {
    MySQL sql("levels_information");
    const TimeIt stime;
    const MySQL::tRows master(sql.master("Master")); // Get mysql master tbl

    std::cout << "loaded " << master.size() << " rows from master in " 
  ...
  void pruneParsers() {
    ...
    for (tRows::const_iterator it(mRows.begin()), et(mRows.end()); it != et; ++it) {
      const tSet& parsers(it->second.parsers());
      for (tSet::const_iterator jt(parsers.begin()), jet(parsers.end()); jt != jet; ++jt) {
        tbls.insert(*jt);
      }
    }
    std::cout << "Looking at " << tbls.size() << " for active ones" << std::endl;
    MySQL sql("levels_data");
    const MySQL::tTables& tables(sql.tables());
    std::cout << "Found " << tables.size() << " tables" << std::endl;
    tSet active;
    const time_t threshold(time(0) - 120 * 86400); // 120 days in the past
    for (MySQL::tTables::const_iterator it(tables.begin()), et(tables.end()); it != et; ++it) {
      const std::string::size_type i(it->find('_'));
      if (i == it->npos) continue; // No underscore found
      const Data::Type type(Data::decodeType(it->substr(0,i)));
      if (type == Data::LASTTYPE) continue; // unknown type
      const std::string name(MySQL::fixId(it->substr(i+1)));
      if (tbls.find(name) == tbls.end()) continue; // Not known
      if (active.find(name) != active.end()) continue; // Already looked at
      if (sql.maxDataTime(*it) < threshold) continue; // Nothing current
      active.insert(name); 
      if (active.size() % 100 == 0) std::cout << "Inserted " << active.size() << " entries" << std::endl;
    }
    std::cout << active.size() << " survived" << std::endl;
    for (tRows::iterator it(mRows.begin()), et(mRows.end()); it != et; ++it) {
      it->second.pruneParsers(active);
    }
  }
  ...
  void saveGuides(MyDB& db) {
    ...
    std::cout << stime << " seconds to save " << cnt << " guides" << std::endl;
  } // saveGuides 

  void saveSources(MyDB& db) {
